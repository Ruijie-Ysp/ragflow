export default {
  translation: {
    ...
    knowledgeDetails: {
      ...
      parsingStatus: '解析状态',
      parsingStatusTip:
        '文本解析的时间取决于诸多因素。如果开启了知识图谱、RAPTOR、自动问题提取、自动关键词提取等功能，时间会更长。如果解析进度条长时间不更新，也可以参考 FAQ。',
      processBeginAt: '开始于',
      ...
      layoutRecognizeTip:
        '使用视觉模型进行 PDF 布局分析，以更好地识别文档结构，找到标题、文本块、图像和表格的位置。 如果选择 Naive 选项，则只能获取 PDF 的纯文本。请注意该功能只适用于 PDF 文档，对其他文档不生效。欲了解更多信息，请参阅相关文档。',
      taskPageSize: '任务页面大小',
      ...
      rerankTip: `非必选项：若不选择 rerank 模型，系统将默认采用关键词相似度与向量余弦相似度相结合的混合查询方式；如果设置了 rerank 模型，则混合查询中的向量相似度部分将被 rerank 打分替代。请注意：采用 rerank 模型会非常耗时。如需选用 rerank 模型，建议使用 SaaS 的 rerank 模型服务；如果你倾向使用本地部署的 rerank 模型，请务必确保使用 GPU 环境启动系统。`,
      topK: 'Top-K',
      ...
      html4excel: '表格转HTML',
      html4excelTip: `与 General 切片方法配合使用。未开启状态下，表格文件（XLSX、XLS（Excel 97-2003））会按行解析为键值对。开启后，表格文件会被解析为 HTML 表格。若原始表格超过 12 行，系统会自动按每 12 行拆分为多个 HTML 表格。欲了解更多详情，请参阅相关文档。`,
      autoKeywords: '自动关键词提取',
      autoKeywordsTip: `自动为每个文本块中提取 N 个关键词，用以提升查询精度。请注意：该功能采用“系统模型设置”中设置的默认聊天模型提取关键词，因此也会产生更多 Token 消耗。另外，你也可以手动更新生成的关键词。详情请见相关文档。`,
      autoQuestions: '自动问题提取',
      autoQuestionsTip: `利用“系统模型设置”中设置的 chat model 对知识库的每个文本块提取 N 个问题以提高其排名得分。请注意，开启后将消耗额外的 token。您可以在块列表中查看、编辑结果。如果自动问题提取发生错误，不会妨碍整个分块过程，只会将空结果添加到原始文本块。详情请见相关文档。`,
...
`,
    ...
    <p>
      ...
      您只需与<i>'知识库和智能体管理系统'</i>交谈即可列出所有符合资格的候选人。
      </p>
        `,
...
`,
      ...
      useRaptorTip: '为多跳问答任务启用 RAPTOR，详情请见：相关文档。',
      prompt: '提示词',
...
以上就是你需要总结的内容。`,
      ...
      entityTypes: '实体类型',
      pageRank: '页面排名',
      pageRankTip: `知识库检索时，你可以为特定知识库设置较高的 PageRank 分数，该知识库中匹配文本块的混合相似度得分会自动叠加 PageRank 分数，从而提升排序权重。详见相关文档。`,
      tagName: '标签',
      frequency: '频次',
      searchTags: '搜索标签',
      tagCloud: '云',
      ...
      useGraphRag: '提取知识图谱',
      useGraphRagTip:
        '基于知识库内所有切好的文本块构建知识图谱，用以提升多跳和复杂问题回答的正确率。请注意：构建知识图谱将消耗大量 token 和时间。详见相关文档。',
      graphRagMethod: '方法',
      graphRagMethodTip: `Light：实体和关系提取提示来自 LightRAG：“LightRAG：简单快速的检索增强生成”<br>
General：实体和关系提取提示来自 graphrag：基于图的模块化检索增强生成 (RAG) 系统`,
      resolution: '实体归一化',
      resolutionTip: `解析过程会将具有相同含义的实体合并在一起，从而使知识图谱更简洁、更准确。应合并以下实体：特朗普总统、唐纳德·特朗普、唐纳德·J·特朗普、唐纳德·约翰·特朗普`,
      community: '社区报告生成',
      communityTip:
        '区块被聚集成层次化的社区，实体和关系通过更高抽象层次将每个部分连接起来。然后，我们使用 LLM 生成每个社区的摘要，称为社区报告。更多信息：请参阅相关资料。',
    },
    chunk: {
      chunk: '解析块',
      bulk: '批量',
    ...
    setting: {
      ...
      ttsModel: 'TTS模型',
      ttsModelTip:
        '默认的tts模型会被用于在对话过程中请求语音生成时使用。如未显示可选模型，请检查可用模型配置。',
      embeddingModel: '嵌入模型',
      embeddingModelTip:
        '所有新创建的知识库使用的默认嵌入模型。如未显示可选模型，请检查可用模型配置。',
      img2txtModel: 'Img2txt模型',
      img2txtModelTip:
        '所有新创建的知识库都将使用默认的 img2txt 模型。它可以描述图片或视频。如未显示可选模型，请检查可用模型配置。',
      sequence2txtModel: 'Speech2txt模型',
      sequence2txtModelTip:
        '所有新创建的知识库都将使用默认的 ASR 模型。使用此模型将语音翻译为相应的文本。如未显示可选模型，请检查可用模型配置。',
      rerankModel: 'Rerank模型',
      rerankModelTip: `默认的 reranking 模型。如未显示可选模型，请检查可用模型配置。`,
      workspace: '工作空间',
      upgrade: '升级',
      addLlmTitle: '添加 LLM',
...
以上就是你需要总结的内容。`,
      ...
      gitHub: 'GitHub',
      gitHubDescription:
        '该组件用于搜索代码仓库。可通过 Top N 指定需要的搜索结果数量。',
      baiduFanyi: '百度翻译',
...
[在此处插入文本]`,
    ...
    corpus: {
      title: '语料库',
      description: '查看所有知识库的语料库统计信息',
      patientDataCorpus: '患者数据语料库',
      documentCorpus: '文献知识语料库',
      vectorCorpus: '向量知识语料库',
      knowledgeGraphCorpus: '知识图谱语料库',
      ...
      selectTable: '选择表',
      selectField: '选择字段',
      selectPrimaryField: '选择主字段',
      connectionSuccess: '连接成功',
      connectionFailed: '连接失败',
      configSaved: '配置已保存',
      saveSuccess: '保存成功',
      saveFailed: '保存失败',
      databaseConfigDescription: '配置医疗数据库连接信息',
      ...
      selectPatientTable: '选择患者表',
      selectOrderTable: '选择医嘱表',
      selectRecordTable: '选择病历表',
      selectExamTable: '选择检查报告表',
      selectLabTable: '选择检验报告表',

      patientTableRequired: '请选择患者表',
      orderTableRequired: '请选择医嘱表',
      recordTableRequired: '请选择病历表',
      examTableRequired: '请选择检查报告表',
      labTableRequired: '请选择检验报告表',

      medicalDataConfig: '医疗数据配置',
      medicalDataConfigDescription: '配置5类医疗数据表及统计字段',
    },
    literature: {
      title: '文献处理',
      upload: '上传文献',
      noData: '暂无文献，请上传PDF文件',
      size: '文件大小',
      createTime: '创建时间',
      processDuration: '处理耗时',
      process: '处理',
      statusPending: '待处理',
      ...
      notFound: '文献不存在',
      waitingForProcess: '等待处理完成...',
      noContent: '暂无内容',
      agentProcessing: '智能体处理',
      processFirst: '请先完成文献处理',
      selectAgent: '选择智能体',
      runAgent: '运行智能体',
      agentResults: '处理结果',
      noAgentResults: '暂无处理结果',
      agentSuccess: '处理成功',
      agentFailed: '处理失败',
      unknownAgent: '未知智能体',
      uploadSuccess: '上传成功',
      uploadFailed: '上传失败',
      processSuccess: '处理请求已提交',
      processFailed: '处理请求失败',
      processStarted: '处理已开始',
      deleteSuccess: '删除成功',
      deleteFailed: '删除失败',
      runAgentSuccess: '智能体运行请求已提交',
      runAgentFailed: '智能体运行失败',
      agentStarted: '智能体处理已开始',
      parsedContent: '解析内容',
      pdfLoadError: 'PDF加载失败',
      deleteResult: '删除结果',
      deleteResultConfirm: '确定要删除此处理结果吗？',
      deleteResultSuccess: '删除成功',
      deleteResultFailed: '删除失败',
      export: '导出',
      exportSuccess: '导出成功',
      exportFailed: '导出失败',
      exportTxt: '文本文件',
      exportPdfHint: '请在打印对话框中选择"另存为PDF"',
      // 解析方法相关
      parseMethod: '解析方法',
      selectParseMethod: '选择解析方法',
      parseMethodMinerU: 'MinerU - 适合复杂版面PDF',
      parseMethodDeepDOC: 'DeepDOC - 内置深度学习解析',
      parseMethodPlainText: 'PlainText - 纯文本提取',
      parseMethodTable: 'Table - 表格优化解析',
      // 取消处理相关
      cancelProcess: '取消处理',
      cancelSuccess: '已取消处理',
      cancelFailed: '取消失败',
      // 输入来源相关
      inputSource: '输入来源',
      selectInputSource: '选择输入来源',
      inputSourceMarkdown: '原始Markdown',
      agentResultsAsInput: '智能体处理结果',
      agentResult: '处理结果',
      contentLength: '内容长度',
      chars: '字符',
      willSegment: '将分{{count}}段处理',
      segmentHint: '内容较长，将自动分段处理后合并结果，确保完整性',
      selectAgentPlaceholder: '选择智能体',
      // 高级选项
      advancedOptions: '高级选项',
      segmentSize: '分段大小',
      segmentSizeDesc: '每段最多{{size}}字符，超出将自动分段处理',
      noSegmentDesc: '不分段，一次性处理全部内容（需要模型支持大上下文）',
    },
  },
};
